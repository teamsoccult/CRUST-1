...
#Working directory of RAW data. 
setwd("/run/media/victormp/VERBATIM HD/data/data_new")
...
datawrite <- function(i){
  ...
  setwd("/run/media/victormp/VERBATIM HD/data/data_prepped-18-11")
...
Test for the new agents and their sampling of models - the test here being that sampling_global.csv should be empty, and that sampling_distribution.csv should have equal distributions for the sampled models. 

```{r}

#working directory. 
setwd("D:/data/data_prepped_25-11")
path = "D:/data/data_prepped_25-11/"
...
sample_global <- function(files){
  ...
  for(i in files){
    ...
    #create DF. 
    df_same_model <- df %>%
      filter(strategy == "research", old_model == selected_model)
    ...
    datalist1[i] <- list(df_same_model)
    datalist2[i] <- list(df_agent_types)
  }
    
  df_final1 <- do.call("rbind", datalist1)
  df_final2 <- do.call("rbind", datalist2)
  
  #directory for the files: 
  setwd("D:/data/data_tests")
  
  #write csv:
  write_csv(df_final1, "sample_global.csv")
  write_csv(df_final2, "sampling_distribution.csv")
...
#working directory. 
setwd("/run/media/victormp/VERBATIM HD/data/data_prepped_24-11")
path = "/run/media/victormp/VERBATIM HD/data/data_prepped_24-11/"
files <- list.files(path = path, pattern="*.csv")
...
violin_extended_rework <- function(files){
  ...
  #directory for the files: 
  setwd("/run/media/victormp/VERBATIM HD/data/data_plotting")
  
  #write csv:
  write_csv(df_final, "violin_extended_colabtest.csv")
