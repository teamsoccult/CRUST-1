...
### potential new prep ###
myfiles = list.files(pattern=c("*COLAB.csv"), full.names=TRUE)
...
str(data) #all grouping should be factor & the rest numeric/integer. 
...
min(check$stud) #10,000
max(check$stud) #10,000
...
#faster but we need to make sure that it works. 
data.table:fwrite(data, ".csv", row.names = FALSE)
