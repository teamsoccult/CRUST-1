...
#selected true (should be constant)
ggplot(IamGood, aes(timestep, selected_true_model)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~strategy)

#final global true (end yp with true?)
ggplot(IamGood, aes(timestep, final_global_true_model)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~strategy)

#change when true - tried subset
tried <- IamGood %>%
  subset(tried > 1)

tried1 <- tried %>%
  subset(selected_true_model == 1) %>%
  mutate(okay = (changed+already)/tried)

tried2 <- tried %>%
  subset(selected_true_model == 0) %>%
  mutate(okay = (changed+already)/tried)

summary(tried1$okay) #91 percent changed if they did not already have true.
summary(tried2$okay) #78 percent changed if they did not already have true.

summary(IamGood$beta1_error)

#plot 
ggplot(tried, aes(x = selected_true_model, y = changed/tried)) +
...
m2 <- read.csv("data/small_epistemic_test/output_m2.csv", sep = ";")
m10 <- read.csv("data/small_epistemic_test/output_m10.csv", sep = ";")
m7 <- read.csv("data/small_epistemic_test/output_m7.csv", sep = ";")


#proportion over time? 
ggplot(m10, aes(timestep, proportion)) +
  geom_point() +
  geom_smooth() +
  geom_hline(data = IamGood, aes(yintercept = 0.5), color = "red", linetype = "dashed", size = 1) 

#proportion over time?
ggplot(m7, aes(timestep, proportion)) +
  geom_point() +
  geom_smooth() +
  geom_hline(data = IamGood, aes(yintercept = 0.5), color = "red", linetype = "dashed", size = 1) 

#proportion over time?
ggplot(m2, aes(timestep, proportion)) +
  geom_point() +
  geom_smooth() +
  geom_hline(data = IamGood, aes(yintercept = 0.5), color = "red", linetype = "dashed", size = 1) 
