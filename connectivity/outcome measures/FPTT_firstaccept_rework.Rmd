...
Now employing a VERY simple method, ##THE MEAN ##. Although this is a very simplistic way of getting the first passage, I think this could be a way of tackling it. However, the way it is measured makes some trials have a first passage below 100. The chunk below automatically assign those trials the value 100. 

```{r}
math <- df %>%
  mutate(most_adhered = ifelse(15 == max.col(.[,c(11:25)], ties.method = "last") & studies >= 100, 1, 0))

#math$identifier <- math %>%
  #str_glue_data("{network} {population} {sigma} {net_size} {colab_cond} {base_SS} {true_model}")

mathing <- math %>% 
  group_by(replica, population, network, sigma, true_model, net_size, base_SS, colab_cond) %>% 
  arrange(replica, population, network, sigma, true_model, net_size, base_SS, colab_cond, studies) %>%
  mutate(hit_studies = studies[min(which(most_adhered == 1))], 
         before_studies = studies[min(which(most_adhered == 1)-1)],
         hit_prop = prop_true[min(which(most_adhered == 1))],
         before_prop = prop_true[min(which(most_adhered == 1)-1)],
         first_passage = round((hit_studies+before_studies)/2),
         first_passage = ifelse(first_passage <= 100, 100, first_passage)) %>% #This final line makes all the means below 100 go to 100. 
  ungroup()
...
tommy <- na.omit(mathing) #VERY PROBLEMATIC, SKEWS THE TRIALS - HOW TO CIRCUMVENT THIS

population_math <- tommy %>%
  group_by(population) %>%
  summarize(FPMA_median = median(first_passage), 
            FPMA_mean = mean(first_passage),
            FPMA_IQR = IQR(first_passage)) 
