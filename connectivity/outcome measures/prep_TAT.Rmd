...
small <- read.csv("small_sanity.csv")
...
TAT <- small %>%  #set the true model
  group_by(replica, network, population, sigma) %>% #needs to be sigma as well. 
...
logic <- as.data.frame(small$proportion_true >= small[,c(36:50)])
logic <- as.data.frame(apply(logic, 2, FUN = as.integer))

#mutating 1 for TAT_c & 0 for not TAT_c
TAT_c <- small %>%
  mutate(TAT_C = ifelse(rowSums(logic) == 15, 1, 0))
  
#per pop, network & replica
TAT_C_table <- TAT_c %>%
  group_by(replica, population, network, sigma) %>% #sigma as well. 
...
```{r}
lattice <- read.csv("lattice_sanity.csv")
...
TAT <- lattice %>%  #set the true model
  group_by(replica, network, population, sigma) %>% #could be sigma as well
...
#checking when proportion is greatest. 
logic <- as.data.frame(lattice$proportion_true >= lattice[,c(36:50)])
logic <- as.data.frame(apply(logic, 2, FUN = as.integer))
