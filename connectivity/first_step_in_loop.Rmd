...
```{r}
library(pacman)
p_load(caTools, data.table, permute, matrixStats, MCMCpack, igraph)
```
...
Backtracking the network.
The question is how we save this. 

```{r}
getwd()
network <- load("connectivity/networks/small_world_tess")
network_variable <- network 

#where are they in the adjacency
doeshework <- names(V(small_world_tess))
doeshework

#the adjacency
areyoualsobad <- as_adjacency_matrix(small_world_tess, sparse = F)
areyoualsobad

```

Checking whether agents actually capture stuff. 

```{r}
#reading file 
getwd()
output <- read.csv("data/output.csv", sep = ";")
