...
source(paste0(scriptDir, "/constants_4_rework2.R")) #new constants baby.
...
source(paste0(scriptDir, "/ABM_TOM_reworkPLAY.R")) #TOM cit. network. 
```
...
```{r}
net_types <- list("TOM", "Lattice") #"Lattice", "Small", "TOM"
sigmas <- list(0.5, 0.8) #0.2, 0.5, 0.8 
base_sampleSizes <- list(20, 100) #20, 100. 
pop_types <- list("All", "Mave") #"Bo", "Mave", "Tess", "All"
tMods <- list(7, 13) #2, 7, 13. 
...
Hard-set for troubleshooting 

```{r}
net_type <- "TOM" #"Lattice", "Small", "TOM"
sigma <- 0.5 #0.2, 0.5, 0.8 
base_sampleSize <- 20 #20, 100. 
pop_type <- "All" #"Bo", "Mave", "Tess", "All"
tMod <- 7 #2, 7, 13. 
modelCompare <- BIC #AIC, BIC 
colab_prob <- 0.02 #no colab vs. colab. 
```
