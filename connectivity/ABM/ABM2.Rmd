...
replications <- 3 #100 originally, now 2. 
...
Combinations which are unstable in terms of finding the true model. 

```{r}
net_types <- list("Lattice", "Small")
sigmas <- list(0.5) #0.2 & 0.8 deleted. 
sampleSizes <- list(100) #20 deleted. 
pop_types <- list("Bo", "Tess", "Mave", "All")
tMods <- list(7, 13)
criterions <- list("BIC", "AIC") #should be modelCompare. 
```
...
for(net_type in net_types){ 
  for(sigma in sigmas){
    for(sampleSize in sampleSizes){
      for(pop_type in pop_types){
        for(tMod in tMods){ #not sure this works. 
          for(criterion in criterions){ #should be 
