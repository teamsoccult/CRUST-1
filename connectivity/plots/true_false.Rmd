...
df_bo <- df %>%
  ...
  mutate(unique_col = as.factor(paste(true_model, sigma)))
...
g <- df_bo %>%
  ...
  geom_hline(aes(yintercept = 0.5), color = "red", linetype = "dotted", size = 1) + 
  labs(x = "Studies", y = "Proportion", color = "Proportion of different models", 
       title = "Model 14 and the true model for Bo-populations",
       subtitle = paste("Network:", network_p, "|", "Population:", population_p)) +
...
# g <- df_bo %>%
#   ggplot(aes(studies, value, fill = variable))+
#   geom_point(alpha = 0.01) +
#   geom_smooth(method = "loess", se = F, alpha = 0.1, size = 1.2, aes(labels = true_model)) +
#   geom_hline(aes(yintercept = 0.5), color = "red", linetype = "dotted", size = 1) + 
#   labs(x = "Studies", y = "Proportion", color = "Proportion of different models", 
#        title = "Model 14 and the true model for Bo-populations",
#        subtitle = paste("Network:", network_p, "|", "Population:", population_p)) +
