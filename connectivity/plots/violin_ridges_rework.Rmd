...
file <- read.csv("mikkelBash.csv", sep = ",")
...
(1) For populations & averaged over everything else. 
Filtering by sample size. Not sure we want to keep this. 
...
g <- melt_RR_TAT %>%
    filter(final_g_true == "Overall",
           sample_size == "Sample size: 20") %>%
    ggplot(aes(x = proportion, y = Sigma, 
                             color = variable, fill = variable)) +
...
#melting again
df_RR_melt <- melt(all_overall, id.vars = c("replica", "Population", "Sigma", "true_model", "sample_size", "final_g_true"), measure.vars = c("RR"), variable.name = "variable", value.name = "proportion")
