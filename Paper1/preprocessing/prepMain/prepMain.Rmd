...
#Naming scheme:
naming_scheme <- "_Reproducibility.csv"

#Working directory 
setwd("~/CRUST-1/Paper1/data/raw/csv")

```

Relabeling orig_type & modelcompare + taking up to turn 10000. 
Outputs the files into: ~/CRUST-1/Paper1/data/preprocessed 
...
data_rename <- function(i){
  ...
  data <- data %>%
    ...
    mutate(orig_type = factor(orig_type, 
                              labels = c("Rey", "Tess", "Bo", "Mave")),
         modelcompare = factor(if_else(modelcompare == 4, "AIC", "BIC")))
  
  #directory for the files: 
  setwd("~/CRUST-1/Paper1/data/preprocessed")
...
Checks that all wanted files are in the proprocessed folder. 
...
#working directory
setwd("~/CRUST-1/Paper1/data/preprocessed")
path = "~/CRUST-1/Paper1/data/preprocessed"
files <- list.files(path = path, pattern = "*.csv")

#testing stuff
df <- read_csv("All_0.2_AIC_100_13_Reproducibility.csv")[mycols] %>%
      mutate(orig_type = factor(orig_type))


test <- read_csv("All_0.2_AIC_100_13_Reproducibility.csv")[mycols]
df <- test %>%
      mutate(orig_type = factor(orig_type),
             old_model = numeric(old_model),
             selected_model = factor(selected_model))

df_total_errors <- df %>% 
        group_by(orig_type) %>%
        summarize(count = sum(old_model == selected_model), #factor error (2020)
                  total = n()) #how many of that agent type in total. 
...
samplingFX <- function(files){
  ...
  for(i in files){
    ...
    #Checks whether they sample their current model (happens in Reproducibility)
    #This is allowed in Devezer et al., but should not happen for "Rework". 
    #count = how many times did they sample their current model. 
    #total = how many times does the agent 
    df_total_errors <- df %>% 
        group_by(orig_type) %>%
        summarize(count = sum(old_model == selected_model), #factor error (2020)
                  total = n())
    
    #where are the errors (which old models)
    df_qualitative_errors <- df %>%
    ...
    datalist1[i] <- list(df_total_errors)
    datalist2[i] <- list(df_qualitative_errors)
    datalist3[i] <- list(df_sampling)
  }
    
  df_total_errors <- do.call("rbind", datalist1)
  df_qualitative_errors <- do.call("rbind", datalist2)
  df_sampling <- do.call("rbind", datalist3)
  
  #directory for the files: 
  setwd("D:/data/dataSampling")
  
  #write csv:
  write_csv(df_total_errors, paste0("df_total_errors", naming_scheme))
  write_csv(df_qualitative_errors, paste0("df_qualitative_errors",
                                          naming_scheme))
...
```{r}
#Naming scheme:
naming_scheme <- "_Devezer_original.csv"

#working directory. 
setwd("D:/data/data_prepped_05-12")
path = "D:/data/data_prepped_05-12"
files <- list.files(path = path, pattern="*.csv")
...
violin_ridges_rework <- function(files){
  ...
  for(i in files){
    ...
    TAT <- df %>%
    group_by(replica, network, population, sigma, true_model, modelcompare, base_SS) %>%
    summarize(TAT = sum(as.numeric(final_g == true_model)) / n())
  ...
  for(i in files){
    ...
    df <- df %>%
      ...
      summarize(agent_away = sum(as.numeric(init_g_true == 1 & 
            ...
            stickiness_overall = 1 - (agent_away / agent_total))
  ...
  for(i in files){
   ...
   #as factor
    ...
    TAT <- df %>%  
      ...
      mutate(TAT = sum(as.numeric(final_g == true_model)) / n(),
        TAT_50 = ifelse(TAT >= 0.5, 1, 0)) %>%
