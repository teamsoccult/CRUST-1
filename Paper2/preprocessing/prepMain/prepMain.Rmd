...
Outcomes <- function(files){
  ...
  for(i in files){
    ...
    df_outcomes <- do.call("rbind", list(df_basic, stickiness, fptt))
    df_outcomes_strat <- do.call("rbind", list(df_strat, stickiness_strat, fptt_strat))
...
violin_ridges_rework <- function(files){
  ...
  write_csv(RR_TAT, paste0("RR_TAT", naming_scheme))
  write_csv(RR_TF, paste0("RR_TF", naming_scheme))
  write_csv(RR_TF_BYSTRAT, paste0("RR_TF_BYSTRAT", naming_scheme))
...
TAT_C_rework.Rmd - currently hardset for the different true models of investigation (2, 7, 13)
...
First passage time to true (true accept): 
