...
#load data 
setwd("E:/data/data_plotting")
df <- read_csv("violin_extended_rework.csv")
df <- df[,-6]

df1 <- read_csv("violin_extended_colabtest.csv")
df2 <- read_csv("violin_extended_strat_colabtest.csv")
...
#as factor
df <- df %>%
  filter(network == "Lattice" | network == "TOM",
         net_size == 484) %>%
...
RR_FUN(df, colab_cond) #looks terrible 
RR_FUN(df, net_size) #looks terrible. 
RR_FUN(df, strategy) #new way of grouping. 
...
RR_FUN3(df, population, colab_cond) #no effect it seems. 
...
RR_FUN3(df, network, colab_cond) #TOM does worst overall. 
...
RR_FUN3(df, sigma, true_model) #interaction effect for sure. 
RR_FUN3(df, sigma, colab_cond) #nothing. 
...
RR_FUN3(df, true_model, colab_cond) #nothing
RR_FUN3(df, true_model, net_size) #nothing. 

#colab condition vs. everything
RR_FUN3(df, colab_cond, network) #nothing
RR_FUN3(df, colab_cond, population) #nothing.
RR_FUN3(df, colab_cond, sigma) #nothing.
RR_FUN3(df, colab_cond, true_model) #nothing. 
RR_FUN3(df, colab_cond, net_size) #nothing. 

#new comparison (strategy vs. replication). 
RR_FUN3(df, strategy, colab_cond)
```
