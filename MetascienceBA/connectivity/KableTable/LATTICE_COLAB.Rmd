...
```{r}
#clearing & reading 
temp = list.files(pattern="*LATTICE_COLAB.csv")
list2env(
...
TAT_MED <- medianFX(TAT_LATTICE_COLAB, population, TAT_median)
TAT50_MED <- medianFX(TAT50_LATTICE_COLAB, population, TAT50_median)
TATC_MED <- medianFX(TATC_LATTICE_COLAB, population, TATC_median)
STICK_MED <- medianFX(stickiness_LATTICE_COLAB, population, stickiness_median)
RR_MED <- medianFX(RR_LATTICE_COLAB, population, RR_median)
RRT_MED <- medianFX(RRT_LATTICE_COLAB, population, RRT_median)

#first passage (differs, so hard coded)
FPTT_MED <- FPTT_LATTICE_COLAB %>%
...
#reusing the functon 
TAT_IQR <- medianFX(TAT_LATTICE_COLAB, population, TAT_IQR)
TAT50_IQR <- medianFX(TAT50_LATTICE_COLAB, population, TAT50_IQR)
TATC_IQR <- medianFX(TATC_LATTICE_COLAB, population, TATC_IQR)
STICK_IQR <- medianFX(stickiness_LATTICE_COLAB, population, stickiness_IQR)
RR_IQR <- medianFX(RR_LATTICE_COLAB, population, RR_IQR)
RRT_IQR <- medianFX(RRT_LATTICE_COLAB, population, RRT_IQR)

#first passage (differs, so hard coded)
FPTT_IQR <- FPTT_LATTICE_COLAB %>%
...
MedIQR %>%
  ...
  add_header_above(c(" ", "Lattice, with collaborations" = 9)) #%>%
  #add_indent(c(2, 5)) 
