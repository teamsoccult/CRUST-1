...
TATC_MED <- medianFX(TATC_LATTICE_413_NOLAB, population, TATC_median)
STICK_MED <- medianFX(stickiness_LATTICE_413_NOLAB, population, stickiness_median)
RR_MED <- medianFX(RR_LATTICE_413_NOLAB, population, RR_median)
RRT_MED <- medianFX(RRT_LATTICE_413_NOLAB, population, RRT_median)
...
TATC_IQR <- medianFX(TATC_LATTICE_413_NOLAB, population, TATC_IQR)
...
RRT_IQR <- medianFX(RRT_LATTICE_413_NOLAB, population, RRT_IQR)
...
```{r}
#clearing & reading 
temp = list.files(pattern="*LATTICE_484_NOLAB.csv")
list2env(
  lapply(setNames(temp, make.names(gsub("*.csv$", "", temp))), 
         read.csv), envir = .GlobalEnv)
```
...
TAT_MED <- medianFX(TAT_LATTICE_484_NOLAB, population, TAT_median)
TAT50_MED <- medianFX(TAT50_LATTICE_484_NOLAB, population, TAT50_median)
TATC_MED <- medianFX(TATC_LATTICE_484_NOLAB, population, TATC_median)
STICK_MED <- medianFX(stickiness_LATTICE_484_NOLAB, population, stickiness_median)
RR_MED <- medianFX(RR_LATTICE_484_NOLAB, population, RR_median)
RRT_MED <- medianFX(RRT_LATTICE_484_NOLAB, population, RRT_median)

#first passage (differs, so hard coded)
FPTT_MED <- FPTT_LATTICE_484_NOLAB %>%
  select(population, FPTT_median) %>%
  spread(population, FPTT_median)
...
rm(list=setdiff(ls(), c("a", "temp", "OutMed", "medianFX", "MedIQR")))
list2env(
  lapply(setNames(temp, make.names(gsub("*.csv$", "", temp))), 
         read.csv), envir = .GlobalEnv)
...
#reusing the functon 
TAT_IQR <- medianFX(TAT_LATTICE_484_NOLAB, population, TAT_IQR)
TAT50_IQR <- medianFX(TAT50_LATTICE_484_NOLAB, population, TAT50_IQR)
TATC_IQR <- medianFX(TATC_LATTICE_484_NOLAB, population, TATC_IQR)
STICK_IQR <- medianFX(stickiness_LATTICE_484_NOLAB, population, stickiness_IQR)
RR_IQR <- medianFX(RR_LATTICE_484_NOLAB, population, RR_IQR)
RRT_IQR <- medianFX(RRT_LATTICE_484_NOLAB, population, RRT_IQR)

#first passage (differs, so hard coded)
FPTT_IQR <- FPTT_LATTICE_484_NOLAB %>%
  select(population, FPTT_IQR) %>%
  spread(population, FPTT_IQR)
...
rm(list=setdiff(ls(), c("a", "temp", "OutMed", "OutIQR", "MedIQR")))
...
#creating the relevant groups 
MedIQR1 <- cbind(OutMed, OutIQR)
MedIQR1 <- add_column(MedIQR1, " " = "", .after = 4)
...
MedIQR1 %>%
  ...
  kable_styling(bootstrap_options = c("striped", 
                                      ...
                                      "responsive")) %>%
  add_header_above(c("", "Median" = 4, " " = 1, "IQR" = 4)) %>%
  add_header_above(c(" ", "Lattice, 484 labs, no collaborations" = 9)) #%>%
  #add_indent(c(2, 5)) 
