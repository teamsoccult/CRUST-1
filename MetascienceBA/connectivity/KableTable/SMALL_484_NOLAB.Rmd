...
```{r}
#clearing & reading 
temp = list.files(pattern="*SMALL_484_NOLAB.csv")
list2env(
...
TAT_MED <- medianFX(TAT_SMALL_484_NOLAB, population, TAT_median)
TAT50_MED <- medianFX(TAT50_SMALL_484_NOLAB, population, TAT50_median)
TATC_MED <- medianFX(TATC_SMALL_484_NOLAB, population, TATC_median)
STICK_MED <- medianFX(stickiness_SMALL_484_NOLAB, population, stickiness_median)
RR_MED <- medianFX(RR_SMALL_484_NOLAB, population, RR_median)
RRT_MED <- medianFX(RRT_SMALL_484_NOLAB, population, RRT_median)

#first passage (differs, so hard coded)
FPTT_MED <- FPTT_SMALL_484_NOLAB %>%
...
#reusing the functon 
TAT_IQR <- medianFX(TAT_SMALL_484_NOLAB, population, TAT_IQR)
TAT50_IQR <- medianFX(TAT50_SMALL_484_NOLAB, population, TAT50_IQR)
TATC_IQR <- medianFX(TATC_SMALL_484_NOLAB, population, TATC_IQR)
STICK_IQR <- medianFX(stickiness_SMALL_484_NOLAB, population, stickiness_IQR)
RR_IQR <- medianFX(RR_SMALL_484_NOLAB, population, RR_IQR)
RRT_IQR <- medianFX(RRT_SMALL_484_NOLAB, population, RRT_IQR)

#first passage (differs, so hard coded)
FPTT_IQR <- FPTT_SMALL_484_NOLAB %>%
...
MedIQR %>%
  ...
  add_header_above(c(" ", "Small-world, 484 labs, no collaborations" = 9)) #%>%
  #add_indent(c(2, 5)) 
