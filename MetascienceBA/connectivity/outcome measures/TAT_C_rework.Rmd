...
df <- read_csv("Lattice_All_0.2_BIC_413_hard_20_2-COLAB.csv")
...
#should only create one of them:: 

df_2$maxprop = names(df_2[,c(8, 10:22)])[apply(df_2[,c(8, 10:22)], 1, which.max)]

df_7$maxprop = names(df_7[,c(8:13, 15:22)])[apply(df_7[,c(8:13, 15:22)], 
                                                  1, which.max)]

df_13$maxprop = names(df_13[,c(8:19, 21:22)])[apply(df_13[,c(8:19, 21:22)], 
                                                    1, which.max)]
...
TAT_C_AVG <- read_csv("TATC_AVG.csv")
TAT_C_REP <- read_csv("TATC_REP.csv")

ggplot(TAT_C_REP, aes()) %>%
