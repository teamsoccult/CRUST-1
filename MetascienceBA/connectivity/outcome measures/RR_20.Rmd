...
#replication overall 
df_overall <- df %>%
  group_by(network, population, sigma, true_model, net_size, final_global_true_model) %>%
  summarize(replicated = sum(edges_replicated) / sum(edges_replication_study),
            number = sum(edges_replication_study)) #just to check. 
...
df_byglobal <- df %>%
  group_by(final_global_true_model, replica, network, population, true_model, net_size) %>%
  summarize(replicated = sum(edges_replicated) / sum(edges_replication_study),
            number = sum(edges_replication_study)) 
