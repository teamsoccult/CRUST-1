...
Bo <- read_csv("Bo_0.5_BIC_100_13.csv")
Mave <- read_csv("Mave_0.8_AIC_100_2.csv")
Tess <- read_csv("Tess_0.2_BIC_100_2.csv")

all <- do.call("rbind", list(Bo, Mave, Tess))
...
RR_all <- all %>%
  ...
  group_by(prop_t_mod, replica, network, population, sigma, true_model, modelcompare, base_SS) %>%
  summarize(RR = mean(replicated))

RR_TAT_SUPER <- merge(RR_TAT, RR_all)
...
test <- read_csv("All_0.2_AIC_20_2.csv")

test <- test %>%
  mutate(t_mod_involved = as.numeric(selected_model == true_model | old_model == true_model))

does_it_work <- test %>%
  filter(t_mod_involved == 1, orig_type == "Rey") %>%
  summarize(mean(final_g_true))


```

Relabeling orig_type & modelcompare
...
data_rename <- function(i){
  ...
  setwd("D:/data/Raw data from Computer Room/02-12/raw")
