...
#-------------------------------------------------------------
library(pacman)
p_load(caTools, data.table, permute, matrixStats, MCMCpack, lattice, tidyverse, directlabels, tibble, reshape2, gridExtra)


#############
## PATHS
#############
baseDir <- "C:/Users/Mikkel/Documents/CRUST-1"
...
## Sigma (Error variance) - trying out 0.8
#sigma <- 0.8 #shitton of error variance to model expectation
#sigmaTxt <- gsub("\\.", "", toString(sigma))
...
Soft strategies for all three agents:
...
Population of the different scientists
...
Ok, so this is a bit wild. 
The loop below takes the previous scores from the model comparison sheet, which has the different model statistics. These model statistics are drawn when one of the models is the true model, i.e. the one which generates the data. The model statistics are then what the global model is (row) and what the proposed model is (column).
...
Having each scientist choose the next model in accordance to the probabilities of the probability matrix.

";" indicates an end of statement. The P with the trillion different dimensions are what we stuff things into.
...
Rearranging for heatmaps - note that here that they change the models to the right numbering again :) 
...
Mean time to first hit on the true model
...
#Grid arranging the previous plots
...
Stationary properties of the system
...
Plots for the summary information:
