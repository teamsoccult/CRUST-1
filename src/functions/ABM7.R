...
ABM7 <- function(replications, turns, models, k, 
  ...
  for(replica in 1:replications){
    ...
    if(net_type == "Small"){
      g <- sample_smallworld(1, 160, 2, 0.05)
    }
    else if(net_type == "Lattice"){
      g <- make_lattice(length = 160, dim = 1, 
                        nei = 2, circular = T, directed = F)
    }
    else if(net_type == "Citation"){
      g <- readRDS("~/CRUST-1/citation_network/g.rds")
    }
    ...
    if(pop_type == "All"){ #epi --> All 
      ...
      V(g)$type <- rep(c("Bo", "Mave", "Tess"), c(154,3,3))
    }
    else if(pop_type == "Tess"){ #tess --> Tess
      V(g)$type <- rep(c("Tess", "Bo", "Mave"), c(154,3,3))
    }
    else if(pop_type == "Mave"){ #mave --> Mave
      V(g)$type <- rep(c("Mave", "Bo", "Tess"), c(154,3,3))
    }
