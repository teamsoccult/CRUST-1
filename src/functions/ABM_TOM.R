...
ABM_TOM <- function(replications, turns, models, k, 
  ...
  for(replica in 1:replications){
    ...
    if(net_type == "Small"){
      g <- sample_smallworld(1, 484, 2, 0.05)
    }
    else if(net_type == "Lattice"){
      g <- make_lattice(length = 484, dim = 1, 
    ...
    if(pop_type == "All"){ #epi --> All 
      ...
      V(g)$type <- rep(c("Bo", "Mave", "Tess"), c(474,5,5))
    }
    else if(pop_type == "Tess"){ #tess --> Tess
      V(g)$type <- rep(c("Tess", "Bo", "Mave"), c(474,5,5))
    }
    else if(pop_type == "Mave"){ #mave --> Mave
      V(g)$type <- rep(c("Mave", "Bo", "Tess"), c(474,5,5))
    }
